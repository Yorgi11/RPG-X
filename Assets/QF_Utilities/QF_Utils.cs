...
namespace QF_Tools.QF_Utilities
{
    ...
    public static class QF_Coroutines
    {
        ...
        Action<T> onUpdate, bool unscaledTime = false, Action<bool> onComplete = null)
        {
        ...
        public static IEnumerator LerpOverSequence<T>(
            ...
            Action<T> onUpdate, bool reverse = false, bool unscaledTime = false, Action<bool> onComplete = null)
        {
            onComplete?.Invoke(false);
            if (sequence == null || sequence.Count < 2 || speed <= 0f)
            {
                onComplete?.Invoke(true);
                yield break;
            }
        ...
        public static IEnumerator LerpOverSequenceOverTime<T>(
            IReadOnlyList<T> sequence, float duration,
            Func<T, T, float, T> lerp, Func<T, T, float> distance,
            Action<T> onUpdate, bool reverse = false, bool unscaledTime = false, Action<bool> onComplete = null)
        {
            onComplete?.Invoke(false);
            if (sequence == null || sequence.Count < 2 || duration <= 0f)
            {
                if (sequence != null && sequence.Count > 0)
                    onUpdate?.Invoke(sequence[reverse ? 0 : sequence.Count - 1]);
            ...
            for (int i = startIndex; i != endIndex; i += step)
                totalDistance += Mathf.Max(0f, distance(sequence[i], sequence[i + step]));
            ...
            for (int i = startIndex; i != endIndex; i += step)
            {
                ...
                float segDist = Mathf.Max(0f, distance(a, b));
                float segDur = duration * (segDist / totalDistance);
                if (segDur <= 0f)
                {
                    onUpdate?.Invoke(b);
                    continue;
        ...
        public static IEnumerator LerpOverTime<T1, T2>(
            ...
            Action<(T1, T2)> onUpdate, bool unscaledTime = false, Action<bool> onComplete = null)
        {
        ...
        public static IEnumerator LerpOverSequence<T1, T2>(
            ...
            Action<(T1, T2)> onUpdate, bool reverse = false, bool unscaledTime = false, Action<bool> onComplete = null)
        {
            onComplete?.Invoke(false);
        ...
        public static IEnumerator LerpOverSequenceOverTime<T1, T2>(
            IReadOnlyList<T1> sequenceT1, IReadOnlyList<T2> sequenceT2, float duration,
            Func<T1, T1, float, T1> lerpT1, Func<T2, T2, float, T2> lerpT2,
            Func<T1, T1, float> distanceT1, Func<T2, T2, float> distanceT2,
            Action<(T1, T2)> onUpdate, bool reverse = false, bool unscaledTime = false, Action<bool> onComplete = null)
        {
            onComplete?.Invoke(false);
            if (sequenceT1 == null || sequenceT2 == null || sequenceT1.Count != sequenceT2.Count || sequenceT1.Count < 2 || duration <= 0f)
            {
                if (sequenceT1 != null && sequenceT2 != null && sequenceT1.Count == sequenceT2.Count && sequenceT1.Count > 0)
                {
                    int finalIdx = reverse ? 0 : sequenceT1.Count - 1;
                    onUpdate?.Invoke((sequenceT1[finalIdx], sequenceT2[finalIdx]));
                }
            ...
            float totalDistance = 0f;
            for (int idx = i; idx != end; idx += step)
            {
                int j = idx + step;
                float d1 = Mathf.Max(0f, distanceT1(sequenceT1[idx], sequenceT1[j]));
                float d2 = Mathf.Max(0f, distanceT2(sequenceT2[idx], sequenceT2[j]));
                totalDistance += Mathf.Max(d1, d2);
            }

            if (totalDistance <= 0f)
            {
                onUpdate?.Invoke((sequenceT1[end], sequenceT2[end]));
                onComplete?.Invoke(true);
                yield break;
            }
            ...
            for (; i != end; i += step)
            {
                ...
                float d1 = Mathf.Max(0f, distanceT1(a1, b1));
                float d2 = Mathf.Max(0f, distanceT2(a2, b2));
                float dur = duration * (Mathf.Max(d1, d2) / totalDistance);
